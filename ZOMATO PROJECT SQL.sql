SELECT * FROM analysis.`zomato data`;
...
from analysis.`zomato data`;

#3.Find the Numbers of Resturants based on City and Country.
select analysis2.`zomato 2`.Countryname,analysis.`zomato data`.city,count(restaurantid)no_of_restaurants
from analysis.`zomato data` inner join analysis2.`zomato 2` 
on analysis.`zomato data`.CountryCode=analysis2.`zomato 2`.CountryID 
group by analysis2.`zomato 2`.Countryname,analysis.`zomato data`.city;
...
from analysis.`zomato data` group by year(datekey_opening),quarter(datekey_opening),monthname(datekey_opening) 
...
select has_table_booking,concat(round(count(has_table_booking)/100,1),"%") percentage from analysis.`zomato data` group by has_table_booking;

# highest rating restaurants in each country
SELECT analysis.`zomato data`.RestaurantName, MAX(Rating)as Highest
FROM analysis.`zomato data`
GROUP BY analysis.`zomato data`.RestaurantName;

select  Countryname,RestaurantName,MAX(Rating) from analysis.`zomato data` inner join analysis2.`zomato 2` on analysis.`zomato data`.CountryCode=analysis2.`zomato 2`.CountryID
group by analysis2.`zomato 2`.Countryname;
